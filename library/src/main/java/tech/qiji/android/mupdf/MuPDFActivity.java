...
public class MuPDFActivity extends Activity implements FilePicker.FilePickerSupport {
  ...
  public void createAlertWaiter() {
    ...
    mAlertTask = new AsyncTask<Void, Void, MuPDFAlert>() {
      ...
      @Override protected MuPDFAlert doInBackground(Void... arg0) {
        if (!mAlertsActive) return null;

        return core.waitForAlert();
      }

      @Override protected void onPostExecute(final MuPDFAlert result) {
  ...
  public void createUI(Bundle savedInstanceState) {
    ...
    mDocView = new MuPDFReaderView(this) {
      @Override protected void onMoveToChild(int i) {
        if (core == null) return;
      ...
      @Override protected void onTapMainDocArea() {
        if (!mButtonsVisible) {
          showButtons();
        } else {
      ...
      @Override protected void onDocMotion() {
        hideButtons();
      }

      @Override protected void onHit(Hit item) {
        switch (mTopBarMode) {
          case Annot:
