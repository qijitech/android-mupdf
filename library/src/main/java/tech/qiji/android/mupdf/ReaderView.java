...
public class ReaderView extends AdapterView<Adapter>
  ...
  public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
    ...
    if (v != null) {
      ...
      if (withinBoundsInDirectionOfTravel(bounds, velocityX, velocityY) && expandedBounds.contains(
          0, 0)) {
  ...
  public boolean onScale(ScaleGestureDetector detector) {
    ...
    if (mReflow) {
      ...
      if (v != null) {
        ...
        if (mLastScaleFocusX >= 0) mXScroll += currentFocusX - mLastScaleFocusX;
        if (mLastScaleFocusY >= 0) mYScroll += currentFocusY - mLastScaleFocusY;
    ...
    if (!mResetLayout) {
      ...
      if (cv != null) {
        ...
        if (HORIZONTAL_SCROLLING) {
          move = cv.getLeft() + cv.getMeasuredWidth() + cvOffset.x + GAP / 2 + mXScroll
              < getWidth() / 2;
        } else {
          move = cv.getTop() + cv.getMeasuredHeight() + cvOffset.y + GAP / 2 + mYScroll
              < getHeight() / 2;
        }
    ...
    if (mCurrent > 0) {
      ...
      if (HORIZONTAL_SCROLLING) {
        ...
        lv.layout((cvLeft + cvRight - lv.getMeasuredWidth()) / 2,
            cvTop - lv.getMeasuredHeight() - gap, (cvLeft + cvRight + lv.getMeasuredWidth()) / 2,
            cvTop - gap);
      }
    }
  ...
  @Override public void setSelection(int arg0) {
